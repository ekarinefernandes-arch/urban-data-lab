...
def plotar_mapa(
    ...
    if usar_quantis and valores.nunique(dropna=True) > 1:
        classes = pd.qcut(
            valores,
            q=min(estilo.numero_classes, valores.nunique(dropna=True)),
            duplicates="drop",
        )
        dados_plot["_classe_mapa"] = classes.cat.codes.replace(-1, pd.NA)
        quantidade = len(classes.cat.categories)
        ...
        formatador = _formatar_moeda if currency else _formatar_numero
        for indice, intervalo in enumerate(classes.cat.categories):
            rotulo = (
                f"{formatador(intervalo.left, None)} – "
